"""Tests that libras3d is functional for i.hyper.albedo standalone mode."""
import os, sys
import numpy as np
import pytest
...
def test_wavelength_sidecar():
    ...
    ras3d.close_cube(h)
    sys.path.insert(0, '/home/yann/dev/i.hyper.albedo')
    import importlib
    m = importlib.import_module('i_hyper_albedo')
    try:
        fn = getattr(m, 'get_all_band_wavelengths')
        result = fn(WYVERN_PATH)
        assert result is not None
        assert len(result) > 0
    except AttributeError:
        pytest.skip('get_all_band_wavelengths not found in module (may have different name)')
    finally:
        os.unlink(sidecar)
